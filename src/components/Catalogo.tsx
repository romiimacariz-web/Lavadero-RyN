    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-brand-card rounded-3xl border border-white/[0.04] shadow-xl shadow-black/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-display font-black text-white">Consola de Configuración</h2>
        ...
        <div className="flex flex-wrap items-center gap-1 bg-[#181822]/60 p-1 rounded-xl border border-white/[0.04] self-start md:self-auto">
          <button
            onClick={() => setActiveSubTab('catalogo')}
            className={`px-3.5 py-2 rounded-lg text-xs font-bold tracking-wide transition flex items-center gap-1.5 cursor-pointer ${
      ...
      {activeSubTab === 'catalogo' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-sm font-mono uppercase text-gray-400 tracking-wider font-bold">Catálogo Vigente de Servicios</h3>
            {!isAdding && !editingId && (
              <button
                onClick={() => {
                  resetForm();
                ...
                className="premium-btn-primary px-4 py-2.5 rounded-xl flex items-center justify-center gap-1.5 font-bold text-xs cursor-pointer"
              >
          ...
          {(isAdding || editingId) && (
            <div className="bg-brand-card rounded-3xl border border-brand-red/30 p-5 space-y-4 animate-scaleUp text-xs font-sans shadow-xl shadow-black/25">
              <div className="flex items-center justify-between border-b border-white/[0.05] pb-2">
                <span className="font-display font-bold text-white text-sm">
                  {editingId ? '✍️ Editar Servicio' : '✨ Agregar Servicio al Catálogo'}
                </span>
                <button onClick={resetForm} className="text-gray-400 hover:text-white cursor-pointer">✕</button>
              </div>

              <form onSubmit={editingId ? handleSaveEdit : handleAdd} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Nombre del Servicio *</label>
                    <input
                      type="text"
                      required
                      value={tipo}
                      onChange={(e) => setTipo(e.target.value)}
                      className="w-full bg-[#181822] border border-white/[0.06] text-white focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50 rounded-xl px-3.5 py-2 text-xs transition duration-150 focus:outline-none font-bold"
                      placeholder="Ej. Lavado Premium Carrocería + Motor"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Costo base ($) *</label>
                    <input
                      type="number"
                      required
                      min="0"
                      ...
                      className="w-full bg-[#181822] border border-white/[0.06] text-brand-success font-mono font-bold focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50 rounded-xl px-3.5 py-2 text-xs transition duration-150 focus:outline-none"
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Descripción o Alcance del Trabajo</label>
                  <textarea
                    value={descripcion}
                    onChange={(e) => setDescripcion(e.target.value)}
                    rows={2}
                    className="w-full bg-[#181822] border border-white/[0.06] text-white focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50 rounded-xl px-3.5 py-2 text-xs transition duration-150 focus:outline-none resize-none"
                ...
                <div className="flex justify-end gap-2">
                  <button
                    type="button"
                    onClick={resetForm}
                    className="bg-[#181822] hover:bg-white/[0.04] border border-white/[0.06] text-white font-bold py-2 px-4 rounded-xl text-xs cursor-pointer transition"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="bg-brand-success hover:bg-green-700 text-white font-black py-2 px-5 rounded-xl text-xs cursor-pointer transition shadow-lg shadow-brand-success/10"
                  >
                    Guardar Servicio
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {state.serviciosCatalogo.map(service => (
              <div key={service.id} className="bg-brand-card border border-white/[0.04] rounded-3xl p-5 flex flex-col justify-between hover:border-white/[0.12] transition duration-200 shadow-xl shadow-black/10">
                <div className="space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    <h4 className="font-display font-black text-white text-sm leading-snug">{service.tipo}</h4>
                    <span className="font-mono font-extrabold text-brand-success bg-brand-success/10 border border-brand-success/20 px-2.5 py-1 rounded-xl text-[11px] shrink-0">
                ...
                <div className="flex gap-2 justify-end pt-4 mt-4 border-t border-white/[0.05]">
                  <button
                    onClick={() => handleStartEdit(service)}
                    className="p-2 bg-[#181822] hover:bg-white/[0.04] border border-white/[0.06] rounded-xl text-gray-400 hover:text-white transition cursor-pointer"
                    title="Editar"
                  >
                  ...
                  <button
                    ...
                    className="p-2 bg-[#181822] hover:bg-brand-red/10 border border-white/[0.06] rounded-xl text-brand-red/80 hover:text-brand-red transition cursor-pointer"
                    title="Eliminar"
                  >
      ...
      {activeSubTab === 'perfil' && (
        <div className="bg-brand-card border border-white/[0.04] rounded-3xl p-6 space-y-6 shadow-xl shadow-black/20 animate-scaleUp">
          ...
          <form onSubmit={handleSaveBusinessConfig} className="space-y-4 text-xs font-sans">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Nombre Comercial del Negocio *</label>
                <input
                  type="text"
                  required
                  value={nombreNegocio}
                  onChange={(e) => setNombreNegocio(e.target.value)}
                  className="w-full bg-[#181822] border border-white/[0.06] text-white focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50 rounded-xl px-3.5 py-2.5 text-xs focus:outline-none font-bold"
                  placeholder="Ej: Lavadero RyN"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">URL del Logo (Opcional)</label>
                <input
                  type="text"
                  value={logoUrl}
                  onChange={(e) => setLogoUrl(e.target.value)}
                  className="w-full bg-[#181822] border border-white/[0.06] text-white focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50 rounded-xl px-3.5 py-2.5 text-xs focus:outline-none"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Dirección Física del Local</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#181822] border border-white/[0.06] text-white focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50 rounded-xl pl-10 pr-3.5 py-2.5 text-xs focus:outline-none"
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Días y Horarios de Atención</label>
                <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Usuario de Instagram (Opcional)</label>
                <div className="relative">
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Enlace/Nombre de Facebook (Opcional)</label>
                <div className="relative">
            ...
            <button
              type="submit"
              className="bg-brand-red hover:bg-red-700 text-white font-extrabold text-xs py-3 px-6 rounded-xl transition uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-brand-red/10 cursor-pointer"
            >
      ...
      {activeSubTab === 'seguridad' && (
        <div className="space-y-6 animate-scaleUp">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Clave de admin */}
            <div className="bg-brand-card border border-white/[0.04] rounded-3xl p-5 space-y-4 text-xs font-sans shadow-xl shadow-black/10">
              <h3 className="font-display font-black text-white text-sm flex items-center gap-2 border-b border-white/[0.05] pb-2">
              ...
              <form onSubmit={handleSavePassword} className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Clave del Sistema</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-[#181822] border border-white/[0.06] rounded-xl pl-10 pr-10 py-2.5 text-xs text-white font-mono focus:outline-none focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50"
                    />
                    <button
                      type="button"
                ...
                <button
                  type="submit"
                  className="bg-[#181822] border border-white/[0.06] hover:bg-white/[0.04] text-white font-bold py-2.5 px-4 rounded-xl flex items-center gap-1.5 transition cursor-pointer"
                >
              ...
              {passwordFeedback && (
                <p className="text-[11px] text-brand-success font-bold mt-1">{passwordFeedback}</p>
              )}
            </div>
            ...
            <div className="bg-brand-card border border-white/[0.04] rounded-3xl p-5 space-y-4 text-xs font-sans shadow-xl shadow-black/10">
              ...
              <form onSubmit={handleSaveWhatsapp} className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Número (código de país sin el +)</label>
                  <input
                    type="text"
                    required
                    value={bWhatsapp}
                    ...
                    className="w-full bg-[#181822] border border-white/[0.06] rounded-xl px-3.5 py-2.5 text-xs text-white font-mono focus:outline-none focus:border-brand-red/50 focus:ring-1 focus:ring-brand-red/50"
              ...
              {whatsappFeedback && (
                <p className="text-[11px] text-brand-success font-bold mt-1">{whatsappFeedback}</p>
              )}
            </div>
          </div>

          {/* Backup operations section */}
          <div className="bg-brand-card border border-white/[0.04] rounded-3xl p-6 space-y-6 shadow-xl shadow-black/20">
            <div className="flex items-start gap-4 border-b border-white/[0.05] pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-sans">
              ...
              {/* Export backup panel */}
              <div className="p-5 bg-[#181822]/40 border border-white/[0.04] rounded-2xl space-y-4">
                ...
                <button
                  ...
                  className="bg-brand-success hover:bg-green-700 text-white font-extrabold text-xs py-3 px-5 rounded-xl transition flex items-center gap-2 uppercase tracking-wider cursor-pointer shadow-lg shadow-brand-success/10"
                >
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="text-xs text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-[#181822] file:text-white hover:file:bg-white/[0.04] file:cursor-pointer"
                    />
                  </div>
                  ...
                  {backupSummary && (
                    <div className="p-3 bg-[#FFC107]/5 border border-brand-warning/20 text-white rounded-xl space-y-3">
                      ...
                      <p className="text-[11px] text-gray-300 leading-relaxed font-sans">{backupSummary}</p>
                      
                      <button
                        type="button"
                        onClick={handleTriggerRestore}
                        disabled={isRestoring}
                        className="w-full bg-brand-red hover:bg-red-700 text-white font-extrabold py-2.5 rounded-xl uppercase tracking-wider transition cursor-pointer shadow-lg shadow-brand-red/10"
                      >
                  ...
                  {restoreFeedback && (
                    <p className={`p-2 rounded-xl text-xs font-bold ${
      ...
      {serviceToDelete !== null && (
        <div id="confirm-delete-catalog-modal" className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-[#0F0F15] rounded-3xl border border-white/[0.04] p-6 space-y-6 shadow-2xl shadow-black/60 text-xs sm:text-sm">
